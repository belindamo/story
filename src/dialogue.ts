const CLAMPY = "./src/assets/clampy_sprites.png";

type CharacterSprite = [string, number, number];
...
const CLAMPY_NORMAL: CharacterSprite = [CLAMPY, 0, 0];
const CLAMPY_HAPPY: CharacterSprite = [CLAMPY, 0, -300];
const CLAMPY_DOUBT: CharacterSprite = [CLAMPY, -300, 0];
const CLAMPY_SWEAT: CharacterSprite = [CLAMPY, -300, -300];
...
interface TextNode {
  ...
  text?: string;
  speakerName?: string;
  characterImg: CharacterSprite
  options?: TextOption[];
  next?: number | typeof FINISH;
}

const DIALOGUE_CONTENT: TextNode[] = [
  {
    id: 1,
...
export { CLAMPY, CLAMPY_NORMAL, CLAMPY_HAPPY, CLAMPY_DOUBT, CLAMPY_SWEAT, FINISH, DIALOGUE_CONTENT, EPILOGUES };
export type { TextNode, TextOption}
